KarafTerminalView = require './karaf-terminal-view'
{CompositeDisposable} = require 'atom'
path = require 'path'

capitalize = (str)-> str[0].toUpperCase() + str[1..].toLowerCase()

module.exports = KarafTerminal =
  subscriptions: null
  terminalViews: []
  focusedTerminal: off
  ...
  activate: (state) ->
    ...
    @subscriptions.add atom.commands.add 'atom-workspace',
      'atom-karaf-terminal:open': =>
        @newTerminal()

    @subscriptions.add atom.commands.add 'atom-workspace',
      'atom-karaf-terminal:pipe-path': =>
        @pipeTerminal 'path'

    ['left', 'right', 'up', 'down'].forEach (direction) =>
      @subscriptions.add atom.commands.add 'atom-workspace',
        "atom-karaf-terminal:split-#{direction}",
  ...
  getColors: ->
    ...
    }} = atom.config.get('atom-karaf-terminal')
    [
      normalBlack, normalRed, normalGreen, normalYellow
      normalBlue, normalPurple, normalCyan, normalWhite
      brightBlack, brightRed, brightGreen, brightYellow
  ...
  createTerminalView: ->
    opts =
      runCommand    : atom.config.get 'atom-karaf-terminal.autoRunCommand'
      shellArguments: atom.config.get 'atom-karaf-terminal.shellArguments'
      titleTemplate : atom.config.get 'atom-karaf-terminal.titleTemplate'
      cursorBlink   : atom.config.get 'atom-karaf-terminal.cursorBlink'
      colors        : @getColors()

    terminalView = new KarafTerminalView opts
  ...
  splitTerminal: (direction) ->
    ...
    activePane.karafTerminalSplits or= {}

    pane = activePane["split#{direction}"] items: [terminalView]
    activePane.karafTerminalSplits[direction] = pane
